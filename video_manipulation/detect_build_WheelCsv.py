...
def process_compressed_video(video_path, output_video_path, csv_path, model):
    ...
    print("Processing every frame (output will match original FPS/duration)")

    # Output using XVID codec — same fps as original, so duration is preserved
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python wheel_tracking.py <path_to_video>")
        sys.exit(1)
    ...
    # Load the AI model ONCE (only need it for this single video)
    ...
    output_csv = os.path.join(output_dir, f"{video_name}_telemetry.csv")

    print(f"\n--- Starting processing for: {video} ---")
    process_compressed_video(video, output_video, output_csv, wheel_model)
    print(f"Finished processing for: {video}. Outputs saved in folder: '{output_dir}/'")